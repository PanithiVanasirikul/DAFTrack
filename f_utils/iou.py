...
def KLD_obb_pair(i, j, bboxes1, bboxes2):
    beta = 1
    gaussian1 = bboxes1[int(i)]
    gaussian2 = bboxes2[int(j)]
    gaussian1_mean = gaussian1[:2].reshape(-1, 1)
    gaussian2_mean = gaussian2[:2].reshape(-1, 1)
    gaussian1_cov_sqr = np.array([[gaussian1[2], gaussian1[4]], [gaussian1[4], gaussian1[3]]])
    gaussian2_cov_sqr = np.array([[gaussian2[2], gaussian2[4]], [gaussian2[4], gaussian2[3]]])
    gaussian1_cov = np.matmul(gaussian1_cov_sqr, gaussian1_cov_sqr) #not sure if it needs to be transposed. However, the output won't be different
    gaussian2_cov = np.matmul(gaussian2_cov_sqr, gaussian2_cov_sqr)
    dist_xy = (1/2)*np.matmul(np.matmul((gaussian1_mean - gaussian2_mean).T,np.linalg.inv(gaussian2_cov)),gaussian1_mean - gaussian2_mean)
    dist_M = (1/2)*np.trace(np.matmul(np.linalg.inv(gaussian1_cov), gaussian2_cov)) + (1/2)*np.log(np.linalg.det(gaussian1_cov)/np.linalg.det(gaussian2_cov)) - 1

    dist_ori = dist_xy + dist_M
    dist_final = (1 - 1/(1 - np.log(1 + np.sqrt(dist_ori))))*beta
    return dist_final

def BD_obb_pair(i, j, bboxes1, bboxes2):
    beta = 1
    gaussian1 = bboxes1[int(i)]
    gaussian2 = bboxes2[int(j)]
    gaussian1_mean = gaussian1[:2].reshape(-1, 1)
    ...
    gaussian_cov_comb = (1/2)*(gaussian1_cov + gaussian2_cov)

    dist_xy = (1/8)*np.matmul(np.matmul((gaussian2_mean - gaussian1_mean).T, np.linalg.inv(gaussian_cov_comb)), gaussian2_mean - gaussian1_mean)
    dist_M = (1/2)*np.log(np.linalg.det(gaussian_cov_comb), np.sqrt(np.linalg.det(gaussian1_cov)*np.linalg.det(gaussian2_cov)))

    dist_ori = dist_xy + dist_M
    dist_final = (1 - 1/(1 - np.log(1 + np.sqrt(dist_ori))))*beta
    return dist_final

def GWD_obb_pair(i, j, bboxes1, bboxes2):
    beta = 1
    gaussian1 = bboxes1[int(i)]
    gaussian2 = bboxes2[int(j)]
    gaussian1_mean = gaussian1[:2].reshape(-1, 1)
    ...
    gaussian1_cov = np.matmul(gaussian1_cov_sqr, gaussian1_cov_sqr) #not sure if it needs to be transposed. However, the output won't be different
    gaussian2_cov = np.matmul(gaussian2_cov_sqr, gaussian2_cov_sqr)
    dist_xy = np.linalg.norm(gaussian1_mean - gaussian2_mean)
    dist_M = np.trace(gaussian1_cov + gaussian2_cov - 2*(sqrtm(np.matmul(np.matmul(gaussian1_cov_sqr, gaussian2_cov), gaussian1_cov_sqr)))) #I'm sure that the np.sqrt is wrong
...
class AssociationFunction:
    ...
    @staticmethod
    def KLD_batch_obb(bboxes1, bboxes2) -> np.ndarray:

        N, M = len(bboxes1), len(bboxes2)

        def wrapper(i, j):
            return KLD_obb_pair(i, j, bboxes1, bboxes2)
        
        iou_matrix = np.fromfunction(np.vectorize(wrapper), shape=(N, M), dtype=int)
        return iou_matrix

    @staticmethod
    def BD_batch_obb(bboxes1, bboxes2) -> np.ndarray:

        N, M = len(bboxes1), len(bboxes2)

        def wrapper(i, j):
            return BD_obb_pair(i, j, bboxes1, bboxes2)
        
        iou_matrix = np.fromfunction(np.vectorize(wrapper), shape=(N, M), dtype=int)
        return iou_matrix
    
    @staticmethod
    def GWD_batch_obb(bboxes1, bboxes2) -> np.ndarray:

        N, M = len(bboxes1), len(bboxes2)

        def wrapper(i, j):
            return GWD_obb_pair(i, j, bboxes1, bboxes2)
    ...
    def _get_asso_func(self, asso_mode):
        ...
        ASSO_FUNCS = {
            ...
            "KLD_obb": self.KLD_batch_obb,
            "BD_obb": self.BD_batch_obb,
            "GWD_obb": self.GWD_batch_obb
        }
